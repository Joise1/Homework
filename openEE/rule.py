...
def detect_events_in_sentence(sent, eventive_synsets):
    token2phrase = {}
    for phrase in sent.phrases:
        ...
        if is_noun(phrase.head_word):
            if is_eventive_noun_phrase(phrase, eventive_synsets):
                tokens = remove_determiners(phrase)
                create_event_nugget(tokens, sent)
            continue
    ...
    for token in sent.tokens:
        ...
        # Nouns
        if is_noun(token):
            if is_eventive_noun(token, eventive_synsets):
                create_event_nugget([token], sent)
            continue
...
def detect_events(dataset, eventive_synsets, log_interval=None):
    start = timer()
    for i, doc in tqdm.tqdm(enumerate(dataset.docs)):
        for sent in doc.sents:
            detect_events_in_sentence(sent, eventive_synsets)
...
def is_eventive_noun(noun, eventive_synsets):
    """
...
def is_eventive_noun_phrase(np, eventive_synsets):
    """
...
def is_eventive_wikipedia_concept(wc, eventive_synsets):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, metavar='str', default='out/test_wsd_phrase.json.gz',
                        help="input file (gzipped JSON)")
    parser.add_argument('--output', type=str, metavar='str', default='out/test_wsd_phrase_rule.json.gz',
    ...
    detect_events(dataset, eventive_synsets)
