...
if __name__ == "__main__":
    ...
    for iteration in tqdm.tqdm(range(args.num_iterations)):
        ...
        if iteration % 100 == 0:
            ...
            with open('cross_entropy.txt', 'a', encoding='utf-8') as f:
                f.write(str(cross_entropy.data.cpu().numpy().item()))
                f.write('\n')
            print('-------------KLD--------------')
            print(kld.data.cpu().numpy())
            with open('kld.txt', 'a', encoding='utf-8') as f:
                f.write(str(kld.data.cpu().numpy().item()))
                f.write('\n')
            print('-----------KLD-coef-----------')
            print(coef)
            with open('kld_coef.txt', 'a', encoding='utf-8') as f:
                f.write(str(coef))
                f.write('\n')
            print('------------------------------')
