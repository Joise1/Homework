...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-file', type=bool, default=False, metavar='NS',
                        help='use file (default: False)')
    #Path to mscoco_test file ---
    parser.add_argument('--test-file', type=str, default='data/test.txt', metavar='NS',
                        help='mscoco_test file path (default: data/test.txt)')
    parser.add_argument('--save-model', type=str, default='./trained_RVAE', metavar='NS',
                        help='trained model save path (default: ./trained_RVAE)')
    parser.add_argument('--output', type=str, default='out/generated_sentences.txt', metavar='NS',
                        help='original sentence and generated sentence. (default: out/generated_sentences.txt)')
    ...
    if not args.use_file:
        ...
        file_1 = open(args.test_file, 'r', encoding='utf-8')
    ...
    for i in range(len(data)):
        single_output = {'original_sentence': '', 'generate_sentence': []}
        if args.use_file:
            if i % 100 == 0:
                print('original sentence:     ' + data[i])
            single_output['original_sentence'] = data[i]
        else:
            ...
            for tt in results:
                ...
                for k in range(n_best):
                    ...
                    if batch_loader.end_token in sen:
                        ...
                        single_output['generate_sentence'].append(sen[:sen.index(batch_loader.end_token)])
                    else:
                        if i % 100 == 0:
                            print('generate sentence:     '+sen)
                        single_output['generate_sentence'].append(sen)
        if i % 100 == 0:
            print('\n')
