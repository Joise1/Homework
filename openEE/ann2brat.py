...
def convert_to_brat(dataset, output_dir):
    doc_id = 0
    result_ann_file = 'out/golden_para_result/result.ann'
        ...
        with open(ann_file, 'w') as fout:
            ...
            for sent in doc.sents:
                for nugget in sent.event_nuggets:
                    ...
                    fout.write(f'Event:T{nugget_id}\t{offset}\t{nugget.text} \n')
                    nugget_id += 1

        with open(result_ann_file, 'a') as fout:
            fout.write(f'sentence {doc_id}:\n')
            for sent in doc.sents:
                fout.write(sent.text + '\n')
            nugget_id = 1
            for sent in doc.sents:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, metavar='str', default='out/golden_para_result/test_wsd_phrase_rule.json.gz',
